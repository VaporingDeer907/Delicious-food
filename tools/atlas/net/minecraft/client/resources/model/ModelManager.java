...
public class ModelManager implements PreparableReloadListener, AutoCloseable {
   ...
   public final CompletableFuture<Void> reload(
      PreparationBarrier p_249079_, ResourceManager p_251134_, ProfilerFiller p_250336_, ProfilerFiller p_252324_, Executor p_250550_, Executor p_249221_
   ) {
      p_250336_.startTick();
      ...
      CompletableFuture<Map<ResourceLocation, List<LoadedJson>>> completablefuture1 = loadBlockStates(p_251134_, p_250550_);
      ...
      Map<ResourceLocation, CompletableFuture<StitchResult>> map = this.atlases.scheduleLoad(p_251134_, this.maxMipmapLevels, p_250550_);
      return CompletableFuture.allOf(Stream.concat(map.values().stream(), Stream.of(completablefuture2)).toArray(CompletableFuture[]::new))
         ...
         .thenCompose(p_252255_ -> p_252255_.readyForUpload.thenApply(p_251581_ -> (ReloadState)p_252255_))
         .<ReloadState>thenCompose(p_249079_::wait)
   ...
   private static CompletableFuture<Map<ResourceLocation, BlockModel>> loadBlockModels(ResourceManager p_251361_, Executor p_252189_) {
      return CompletableFuture.<Map>supplyAsync(() -> ModelBakery.MODEL_LISTER.listMatchingResources(p_251361_), p_252189_)
         .thenCompose(
            p_250597_ -> {
                  ...
                  list.add(CompletableFuture.supplyAsync(() -> {
                     try {
                        ...
                        try (Reader reader = entry.getValue().openAsReader()) {
                           var2x = Pair.of(entry.getKey(), BlockModel.fromStream(reader));
                        }
   ...
   private static CompletableFuture<Map<ResourceLocation, List<LoadedJson>>> loadBlockStates(ResourceManager p_252084_, Executor p_249943_) {
      return CompletableFuture.<Map>supplyAsync(() -> ModelBakery.BLOCKSTATE_LISTER.listMatchingResourceStacks(p_252084_), p_249943_)
         .thenCompose(
            p_250744_ -> {
               List<CompletableFuture<Pair<ResourceLocation, List<LoadedJson>>>> list = new ArrayList<>(p_250744_.size());

               for (Entry<ResourceLocation, List<Resource>> entry : p_250744_.entrySet()) {
                  list.add(CompletableFuture.supplyAsync(() -> {
                     List<Resource> list1 = entry.getValue();
                     List<LoadedJson> list2 = new ArrayList<>(list1.size());

                     for (Resource resource : list1) {
                        try (Reader reader = resource.openAsReader()) {
                           JsonObject jsonobject = GsonHelper.parse(reader);
                           list2.add(new LoadedJson(resource.sourcePackId(), jsonobject));
   ...
   private ReloadState loadModels(ProfilerFiller p_252136_, Map<ResourceLocation, StitchResult> p_250646_, ModelBakery p_248945_) {
      p_252136_.push("load");
      ...
      p_248945_.bakeModels((p_251469_, p_251262_) -> {
         StitchResult atlasset$stitchresult = p_250646_.get(p_251262_.atlasLocation());
      ...
      ForgeHooksClient.onModifyBakingResult(p_248945_.getBakedTopLevelModels(), p_248945_);
      ...
      CompletableFuture<Void> completablefuture = CompletableFuture.allOf(
         p_250646_.values().stream().map(StitchResult::readyForUpload).toArray(CompletableFuture[]::new)
      );
      p_252136_.pop();
      p_252136_.endTick();
   ...
   private void apply(ReloadState p_248996_, ProfilerFiller p_251960_) {
      ...
      p_248996_.atlasPreparations.values().forEach(StitchResult::upload);

...
public class SpriteLoader {
   ...
   public static CompletableFuture<List<SpriteContents>> runSpriteSuppliers(List<Supplier<SpriteContents>> p_261516_, Executor p_261791_) {
      List<CompletableFuture<SpriteContents>> list = p_261516_.stream()
         .map(p_261395_ -> CompletableFuture.supplyAsync((Supplier<SpriteContents>)p_261395_, p_261791_))
         .toList();
   ...
   public CompletableFuture<Preparations> loadAndStitch(ResourceManager p_262108_, ResourceLocation p_261754_, int p_262104_, Executor p_261687_) {
      return CompletableFuture.<List<Supplier<SpriteContents>>>supplyAsync(() -> SpriteResourceLoader.load(p_262108_, p_261754_).list(p_262108_), p_261687_)
         .thenCompose(p_261390_ -> runSpriteSuppliers((List<Supplier<SpriteContents>>)p_261390_, p_261687_))
   ...
   public static SpriteContents loadSprite(ResourceLocation p_251630_, Resource p_250558_) {
      ...
      try {
         ...
         LOGGER.error("Unable to parse metadata from {}", p_251630_, var8);
         return null;
      }
      ...
      try (InputStream inputstream = p_250558_.open()) {
         ...
         LOGGER.error("Using missing texture, unable to load {}", p_251630_, var10);
         return null;
      }
      ...
      if (Mth.isMultipleOf(nativeimage.getWidth(), framesize.width()) && Mth.isMultipleOf(nativeimage.getHeight(), framesize.height())) {
         SpriteContents contents = ForgeHooksClient.loadSpriteContents(p_251630_, p_250558_, framesize, nativeimage, animationmetadatasection);
         return contents != null ? contents : new SpriteContents(p_251630_, framesize, nativeimage, animationmetadatasection);
      } else {
         LOGGER.error(
            "Image {} size {},{} is not multiple of frame size {},{}",
            new Object[]{p_251630_, nativeimage.getWidth(), nativeimage.getHeight(), framesize.width(), framesize.height()}
         );
         nativeimage.close();
         return null;
      }
   ...
   private Map<ResourceLocation, TextureAtlasSprite> getStitchedSprites(Stitcher<SpriteContents> p_276117_, int p_276111_, int p_276112_) {
      ...
      p_276117_.gatherSprites(
         (p_251421_, p_250533_, p_251913_) -> {
            TextureAtlasSprite sprite = ForgeHooksClient.loadTextureAtlasSprite(
               this.location, p_251421_, p_276111_, p_276112_, p_250533_, p_251913_, p_251421_.byMipLevel.length - 1
            );
            if (sprite != null) {
